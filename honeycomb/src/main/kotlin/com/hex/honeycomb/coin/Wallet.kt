package com.hex.honeycomb.coin
...
data class Wallet(
    ...
    companion object {
        fun create(comb: Honeycomb): Wallet {
            val generator = KeyPairGenerator.getInstance("RSA")
            generator.initialize(2048)
            val keyPair = generator.generateKeyPair()

            return Wallet(keyPair.public, keyPair.private, comb)
        }
    }

    private val balance: Int get() {
        return getMyTransactions().sumBy { it.amount }
    }

    private fun getMyTransactions() : Collection<TransactionOutput> {
        return comb.UTXO.filterValues { it.isMine(publicKey) }.values
    }
